...
# 建立数据集
# sample_length 输入需要的样本数量。需要多少生成多少 128
# vocab 词表
# window_size 样本长度
# corpus 语料字符串
...
def train(corpus_path, save_weight=True):
    ...
    for epoch in range(epoch_num): # 训练多少轮
        model.train()
        watch_loss = []
        for batch in range(int(train_sample / batch_size)):   # 每轮训练多少次 79次
    ...
    if not save_weight:
        ...
        # os.path.basename这个函数从corpus_path中提取文件名将文件名中的所有"txt"替换为"pth"
        base_name = os.path.basename(corpus_path).replace("txt", "pth")
        # print('base_name', base_name)  # corpus.pth
        # 将字符串"model"和base_name连接起来，形成一个完整的文件路径。这个路径通常指向一个目录，用于存储模型文件
        model_path = os.path.join("model", base_name)
        # print('model_path', model_path)  # model\corpus.pth
...
if __name__ == "__main__":
    ...
    train(r"D:\TTT\NLP算法\预习\第十周 文本生成\week10 文本生成问题\lstm语言模型生成文本/corpus.txt", True)
