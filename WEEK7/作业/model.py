...
class TorchModel(nn.Module):
    ...
    def forward(self, x, target=None):
        ...
        # isinstance 函数检查 x 是否是一个元组
        if isinstance(x, tuple):  # RNN类的模型会同时返回隐单元向量，我们只取序列结果
            # print('x', x[0].shape)  # torch.Size([12, 30, 768])
            x = x[0]
        # 可以采用pooling的方式得到句向量
...
if __name__ == "__main__":
    ...
    print('sequence_output', sequence_output.shape)  # torch.Size([2, 5, 768]) 第一个维度是批次大小/句子数量 （batch size），第二个维度是序列长度（sequence length），第三个维度是隐藏层大小（hidden size）
    print('pooler_output', pooler_output.shape)  # torch.Size([2, 768]) 第一个维度是是句子的数量，第二个维度是隐藏层大小
