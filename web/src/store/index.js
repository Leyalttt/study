...
const store = new Vuex.Store({
  ...
  actions: {
    ...
    initMessage({ commit }) {
      // 监听H5预览页面高度变化
      //  pageHeightChange type也是crs定义的
      messager.on('pageHeightChange', data => {
        // console.log('从H5更新组件高度为', data)
        let height = data.height ? data.height + 72 : 768 //页面的高度
        let list = data.componentsTopList || [] //所有页面中组件高度的集合
